...
1. Resolve the target before the first mutation. If a workflow is already open, edit it in place. Otherwise reuse an existing `workflowId`; for a new workflow, pass `workspaceTarget: "personal"` when the user requests personal space or `workspaceTarget: "active"` only when the user explicitly chooses the current workspace. If neither destination is explicit, ask instead of guessing; omitting both fields fails closed.
